    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      <div className="bg-white rounded-3xl border border-amber-100 p-6 shadow-sm flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-serif font-bold text-maroon-900 mt-1">Notification Center</h1>
          <p className="text-xs text-gray-500">Track book allocation approvals, return confirmations, and settlement status updates.</p>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-amber-100 shadow-sm overflow-hidden divide-y divide-gray-100">
        {loading ? (
          <div className="p-8 text-center text-xs text-amber-900 font-bold">Loading notifications...</div>
        ) : notifications.length > 0 ? (
          notifications.map((n) => (
            <div key={n.id} className={`p-5 flex items-start space-x-4 ${n.read ? 'bg-white' : 'bg-amber-50/50'}`}>
              <div className="p-2.5 rounded-2xl bg-amber-100 text-saffron-700 shrink-0">
          ...
          <div className="p-8 text-center text-xs text-gray-500">No notifications found.</div>
        )}
      </div>
    </div>
  );
