...
export const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fadeIn">
      <div
        className={`bg-white rounded-3xl shadow-2xl border border-amber-100 w-full ${widthMap[maxWidth]} overflow-hidden transform transition-all`}
      ...
      >
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-amber-50 to-orange-50">
          <h3 className="text-lg font-bold font-serif text-maroon-900">{title}</h3>
          <button
            onClick={onClose}
